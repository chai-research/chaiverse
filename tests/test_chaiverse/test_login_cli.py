...
from chaiverse.login_cli import auto_authenticate, login, logout
...
class TestLoginCli:
    ...
    def test_chaiverse_login_from_terminal(self):
        self.runner.invoke(login, env = RUNNER_ENVIRONMENT, input = 'dummy-key\n')
        with open(KEY_PATH) as f:
            assert f.read() == 'dummy-key'

    def test_chaiverse_login_from_terminal_overrides_existing_key_file(self, key_file):
        self.runner.invoke(login, env = RUNNER_ENVIRONMENT, input = 'dummy-key\n')
        with open(KEY_PATH) as f:
            assert f.read() == 'dummy-key'

    def test_chaiverse_login_from_terminal_ignores_empty_key(self):
        result = self.runner.invoke(login, env = RUNNER_ENVIRONMENT, input = '\n')
        assert result.exit_code == 0

    def test_chaiverse_logout_terminal_removes_key(self, key_file):
