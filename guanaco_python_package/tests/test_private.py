...
from chai_guanaco import private
...
def test_client(mock_post, mock_submission):
    response = private.submit_blend(mock_submission, developer_key="mock-key")
...
def test_submit_client_posts_with_correct_payload(mock_post, mock_submission):
    private.submit_blend(mock_submission, developer_key="mock-key")
    headers = {"Authorization": "Bearer mock-key"}
    expected_url = private.get_url(private.SUBMISSION_ENDPOINT)
